...
读、写、批量写、上传四条路径均已对生产环境验证：

- 单张创建后读回内容**逐字节一致**，`[Pic#ID/…]` 引用被服务端正确解析进 `card.files`
- `batch` 一次写入 3 张（英语句子卡 ×2、公式卡 ×1）：3/3 成功，
  **章节顺序与文件顺序完全一致**，逐张内容逐字节一致，`--start N` 续传确认不重复建卡
- `order` 实测为 0 基插入索引（已有 5 张卡的章节传 `order=2`，新卡精确落在索引 2）
