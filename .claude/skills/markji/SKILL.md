...
把一份材料（Markdown、讲义、笔记）做成整套卡片时，按这个顺序走：

1. **读全文，先切知识点**，再想卡片形态。跳过例题、习题、练习章节，除非用户明确要求收进去。
2. **先把所有图片传完**，把 `路径 → file.id` 记进一份 manifest（JSON），后面统一回填。
   不要边建卡边传图——传图失败会留下半套卡。
3. **逐张 `lint`，全部通过后再逐张 `create`**，请求间隔 ≥0.5s。
   顺序处理：**往非空章节追加时直接省略 `--order`**（按提交顺序追加到末尾）。
...
`--file -` 读标准输入，配合 heredoc 一步到位。写入前先 `--dry-run` 给用户过目。

### 房屋风格（务必沿用）

用户 Speaking 系列 177 张卡里 176 张是同一套结构，新卡必须对齐：
...
- 关键表达用 `[T#B,!e53935#…]`（加粗＋红），**每张 2–4 处**，标满等于没标。
- **屈折后缀留在标签外**：写 `[T#B,!e53935#crash]ed`，不要把 `crashed` 整个包进去——
  这样标出的是可迁移的词元，不是这一句里的特定形态。
- 有生词才加音标行 `[T#!888888#/ˈɪnfrəstrʌktʃər/]`，没有就删掉整行。
- `[T#U#常用短语]` 区逐条列出标红过的表达，格式固定 `[P#L#[T#B,U#表达] — 释义]`，
  破折号用半角 `—` 两侧各一空格。释义里可补搭配说明，如「后接名词/动名词」。
- 一句话能同时给出多种地道说法时，用 ` / ` 在同一行并列，例如
  `Knode-recall [T#B,!e53935#crash]ed. / The service went down.`

模板见 `templates/sentence-en.txt`。
