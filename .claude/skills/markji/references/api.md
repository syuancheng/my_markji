...
- `order` 可选，是**0 基插入索引**（已实测）。不传则追加到章节末尾（已实测）。

  实测方法：在一个已有 5 张卡的章节里以 `order: 2` 建卡，新卡精确落在索引 `2`，
  原有 5 张卡的相对顺序不变。

  ⚠️ **章节非空时要当心**：`order` 是相对**整个章节**的绝对索引，不是「本批第几张」。
  往已有 N 张卡的章节追加一批新卡，若从 `0` 开始递增，新卡会全部插到**旧卡前面**。
  正确做法二选一：
  - 直接**省略 `order`**，逐张创建即按提交顺序追加到末尾（最省事，推荐）；
  - 或从 `N` 开始递增（`N` 取自 `chapter.card_ids` 的长度）。
